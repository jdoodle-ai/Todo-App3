    ...
    if task:
        try:
            response = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are a helpful assistant that breaks down tasks into smaller steps."},
                    {"role": "user", "content": f"Break down this task into a maximum of 8 steps: {task.title}"}
                ]
            )
