...
@app.route('/generate_task_breakdown/<int:task_id>', methods=['POST'])
def generate_task_breakdown(task_id):
    task = Task.query.get(task_id)
    if not task:
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Provide a 2-3 sentence breakdown of steps required to complete this task: {task.title}"}
            ],
            max_tokens=150
        )
